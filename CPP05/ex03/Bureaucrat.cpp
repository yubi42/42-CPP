...
{
    std::cout << "Trying to hire bureaucrat: " << _name << std::endl;
    testGrade(_grade);
...
void Bureaucrat::signForm(AForm& AForm)
{
    try
    {
        std::cout <<  "Trying to sign "<< AForm << " with " << *this << std::endl;
        AForm.beSigned(*this);
        std::cout << "\"" << _name << "\" signed \"" << AForm.getName() << "\"." << std::endl;
    }
    catch(const std::exception& e)
    {
        std::cerr << "\"" << _name << "\" could not sign \"" << AForm.getName() << "\" because \"" << e.what() << "\"." << std::endl;
    }  
}
...
void Bureaucrat::executeForm(AForm& AForm)
{
    try
    {
        std::cout << "Trying to execute "<< AForm << " with " << *this << std::endl;
        AForm.execute(*this);
